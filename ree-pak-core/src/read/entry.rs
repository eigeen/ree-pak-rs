    ...
    R: BufRead,
{
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        ...
        if self.magic_read_length < 8 {
            let start = self.magic_read_length;
            let end = 8.min(start + len);
            self.magic_buf[start..end].copy_from_slice(&buf[..(end - start)]);
            self.magic_read_length += len;
        }
...
impl PakEntryReader<Cursor<Vec<u8>>> {
        ...
        R1: Read + Seek,
    {
        ...
        // decrypt single resource encryption
        if entry.encryption_type() != EncryptionType::None {
            let decrypted_data = pak::decrypt_resource_data(&mut &data[..])?;
            data = decrypted_data;
        }
    ...
    // pub fn from_part_reader<R1>(mut part_reader: R1, entry: &PakEntry) -> Result<Self>
    // where
    //     R1: Read + Seek,
    // {
    //     let mut data = vec![0; entry.compressed_size() as usize];
    //     part_reader.read_exact(&mut data)?;
